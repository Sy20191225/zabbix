#!/bin/bash
#监控区块链出块的脚本
#nohup sh /srv/zabbix-agent/script/produce.sh >> /srv/zabbix-agent/var/blockOutNum.stats &
...
is_process || { echo "produce.sh没有运行，请先输入nohup sh /srv/zabbix-agent/script/produce.sh >> /srv/zabbix-agent/var/blockOutNum.stats &命令运行监本。"; exit 1; }
